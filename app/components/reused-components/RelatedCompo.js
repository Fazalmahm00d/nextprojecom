...
function addToWishHandler(img,name,desc,price){
    ...
    return(
        <Link key={props.items.id} href={`/products/${props.productId}`}><div key={props.items.id} className="group relative cursor-pointer lg:h-[28rem] sm:h-[18rem]">
                                    ...
                                    <div className="sm:relative h-[40%] lg:h-[30%] bg-[#F4F5F7] lg:p-4 p-2">
                                        <div className="font-bold lg:text-xl sm:text-l" >{props.items.name}</div>
                                        <p className="text-[#898989] text-sm font-light mt-1 tracking-wide">{props.items.desc}</p>
                                        <p className="relative sm:absolute sm:pl-2 sm:bottom-2  sm:left-0 font-bold lg:mt-2  sm:mt-1 lg:text-xl sm:text-sm tracking-wide">Rs.{props.items.price}</p>
                                    </div> 
