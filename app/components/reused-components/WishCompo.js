import { deleteWishItem } from "@/app/lib/api";
import { useMutation } from "@tanstack/react-query";
import { useSelector } from "react-redux";
import { queryClient } from "../redux-components/reduxProvider";

function WishCompo(props){
    const isEmail=useSelector((state)=>state.authReducer.isEmail);

    const deleteWishItemMutation=useMutation({
    ...
    const deleteWishItemHandler=(id)=>{
        const obj={
            isEmail,
            id
        }
        if(!deleteWishItemMutation.isPending){
            deleteWishItemMutation.mutate(obj)
    
        }
    
       }
    return(
        <div key={props.item._id} className="flex justify-between items-center text-base mt-3">
                                                            <img src={props.item.img} className="h-16 w-16 lg:h-24 lg:w-24 rounded-lg"></img>
                                                            <div className="flex flex-col items-end gap-2">
                                                            <h3 className="font-bold text-xl">{props.item.name}</h3> 
                                                            <p className="text-[#B88E2F] tracking-widest">${props.item.price}</p>
                                                            </div>
                                                            {
                                                                deleteWishItemMutation.isPending ? <button className="animate-spin h-4 w-4 border-t-transparent border-gray-800 rounded-full border-2"></button> :<button onClick={()=>deleteWishItemHandler(props.item._id)}>
